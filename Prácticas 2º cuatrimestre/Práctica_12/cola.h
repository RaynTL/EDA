...
public:
	...
	**/
	...
	O(n) debido a que hay que recorrer en el peor caso todo los nodos hasta el último y de ahí reasignar los punteros _prim y _ult además de los auxiliares ( act y ant )
	**/
	...
	void llevarAlPrincipio(unsigned int pos) {
		...
		if (pos > _nelems || pos < 1)
			throw EPosicionInvalida();
		else if ( pos > 1 )
		{
			Nodo *act = _prim, *ant = NULL;
