...
public:
	...
	long getNumer(){ return _numer; }
	long getDenom(){ return _denom; }

	Racional suma( Racional rac );
	Racional& operator- (const Racional& rac);
	Racional& operator*= (const Racional& rac);
	Racional divideYActualiza(Racional rac);
	bool operator== (const Racional& rac);
